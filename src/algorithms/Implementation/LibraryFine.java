...
public class LibraryFine {
    ...
    static int libraryFine(int d1, int m1, int y1, int d2, int m2, int y2) {
        GregorianCalendar returnedDate = new GregorianCalendar(y1, m1 - 1, d1);
        GregorianCalendar expectedDate = new GregorianCalendar(y2, m2 - 1, d2);
        if (returnedDate.before(expectedDate)) {
            return 0;
        } else if (y2 == y1 && m2 == m1){
            return 15 * (d1 - d2);
        } else if (y2 == y1) {
